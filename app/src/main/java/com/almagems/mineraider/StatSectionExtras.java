...
public final class StatSectionExtras extends StatSectionBase {
	...
	public void init() {
        ...
        texts[0].init("Shared Matches: " + scoreCounter.getSharedMatchCount(), textColor, textColor, fontScale);
        texts[0].pos.tx = x;
        texts[0].pos.ty = y;
        texts[0].posYorigin = texts[0].pos.ty;

        y -= 0.2f;
        texts[1].init("Highest Combos: " + scoreCounter.getHighestComboCount(), textColor, textColor, fontScale);
        texts[1].pos.tx = x;
        texts[1].pos.ty = y;
        texts[1].posYorigin = texts[1].pos.ty;

        y -= 0.2f;
        texts[2].init("Perfect Swaps: " + scoreCounter.getPerfectSwapCount(), textColor, textColor, fontScale);
        texts[2].pos.tx = x;
        texts[2].pos.ty = y;
        texts[2].posYorigin = texts[2].pos.ty;

        y -= 0.2f;
        texts[3].init("Hints: " + scoreCounter.getHintCount(), textColor, textColor, fontScale);
        texts[3].pos.tx = x;
        texts[3].pos.ty = y;
